    ...
    test "with metadata" do
      for metadata <- [%{"meta" => "data"}, [meta: "data"]] do
        for event <- ["event_name", :event_name] do
          ...
          assert String.contains?(output, "count#event_name=1")
          assert String.contains?(output, "meta=data")
      ...
      for context <- [%{"context" => "global"}, [context: "global"]] do
        ...
        for metadata <- [%{"meta" => "data"}, [meta: "data"]] do
          for event <- ["event_name", :event_name] do
            output = line(fn -> Metrix.count(metadata, event, 23) end)
            assert matches_count?(output, event)
            assert String.contains?(output, "count#event_name=23")
            assert String.contains?(output, "meta=data")
            assert String.contains?(output, "context=global")
          end
        end
    ...
    test "with metadata" do
      for metadata <- [%{"meta" => "data"}, [meta: "data"]] do
        for event <- ["event_name", :event_name] do
          ...
          assert String.contains?(output, "sample#event_name=13.4mb")
          assert String.contains?(output, "meta=data")
  ...
  describe "measure" do
    ...
    test "with prefix" do
      ...
      for event <- ["event_name", :event_name] do
        ...
        assert matches_measure?(output, event, "prefix-"),
    ...
    test "with pre-computed latency" do
      for event <- ["event_name", :event_name] do
        ...
        assert matches_measure?(output, event), "Unexpected output format \"#{output}\""
        assert String.contains?(output, "=0.912ms"), "Incorrect measurement value"
      end
    end
    ...
    test "with pre-computed latency and metadata" do
      for metadata <- [%{"meta" => "data"}, [meta: "data"]] do
        for event <- ["event_name", :event_name] do
          ...
          assert matches_measure?(output, event), "Unexpected output format \"#{output}\""
          assert String.contains?(output, "meta=data")
          assert String.contains?(output, "=12.34ms"), "Incorrect measurement value"
        end
      end
    end
    ...
    test "with map metadata passed to function" do
      ...
      output =
        line(fn ->
          Metrix.measure(metadata, "event.name", fn %{"meta" => _data} -> :timer.sleep(1) end)
        end)
    ...
    test "measure pre-computed latency with metadata and global context" do
      ...
      for metadata <- [%{"meta" => "data"}, [meta: "data"]] do
        ...
        assert matches_measure?(output, "event.name"), "Unexpected output format \"#{output}\""
        assert String.contains?(output, "meta=data")
        assert String.contains?(output, "=78ms"), "Incorrect measurement value"
      end
    end
  end
end
